...
function startNight(room) {
  ...
  // Firebase drops empty objects/arrays and null fields — use sentinel values
  // so these fields survive the round-trip and are never undefined on read-back.
  room.night = {
    step: 'pending',          // will be set below
    guardTarget: '_none',     // sentinel: no guard action yet
    wolfVotes: { _init: 1 },  // sentinel: Firebase won't drop a non-empty object
    wolfConfirmedBy: { _init: 1 },
    wolfKill: '_none',
    seerCheck: '_none',
    witchAction: '_none',
    timerStart: Date.now(),
    deaths: ['_init'],        // sentinel for array
    wolfChat: ['_init'],
  };
...
function renderMyNightAction(room, n, me, content) {
  ...
  if (n.step === 'guard') {
    ...
    const last = (room.lastGuardTarget && room.lastGuardTarget !== '_none') ? room.lastGuardTarget : null;
    const guardDone = n.guardTarget !== undefined && n.guardTarget !== '_none';
  ...
  if (n.step === 'wolf') {
    ...
    const wolfVotes = n.wolfVotes || {};
    const wolfConfirmedBy = n.wolfConfirmedBy || {};
    const myVote = wolfVotes[myId];
    // Show all wolves' picks for transparency
    const teamStatus = wolves.map(w => {
      const v = wolfVotes[w.id];
      const confirmed = wolfConfirmedBy[w.id];
      const targetName = (v && v !== '_init') ? room.players.find(p => p.id === v)?.nickname : '未選';
    ...
    const allVoted = wolves.every(w => wolfVotes[w.id] && wolfVotes[w.id] !== '_init');
    const targets = wolves.map(w => wolfVotes[w.id]).filter(v => v && v !== '_init');
    ...
    content.innerHTML = `<div class="card mb-12">
      ...
      ${aliveTargets.filter(p => p.role !== 'wolf').map(p => {
        ...
        const cnt = Object.entries(wolfVotes).filter(([k, t]) => k !== '_init' && t === p.id).length;
        const iConfirmed = wolfConfirmedBy[myId];
    ...
    <div class="card" id="wolf-chat-box">
      <div style="font-size:0.78rem;font-weight:700;color:#fca5a5;letter-spacing:0.5px;margin-bottom:8px;">🐺 狼人私聊</div>
  ...
  if (n.step === 'witch') {
    const wolfKillId = (n.wolfKill && n.wolfKill !== '_none') ? n.wolfKill : null;
    const wolfTarget = wolfKillId ? room.players.find(p => p.id === wolfKillId) : null;
    const done = n.witchAction !== undefined && n.witchAction !== '_none';
...
function guardChoose(targetId) {
  ...
  const last = (room.lastGuardTarget && room.lastGuardTarget !== '_none') ? room.lastGuardTarget : null;
  if (last === targetId) { toast('不能連續兩晚守護同一人'); return; }
  const guardDone = room.night.guardTarget !== undefined && room.night.guardTarget !== '_none';
  if (guardDone) return; // already confirmed
...
function guardConfirm() {
  ...
  if (!room.night || room.night.step !== 'guard') return;
  const last = (room.lastGuardTarget && room.lastGuardTarget !== '_none') ? room.lastGuardTarget : null;
  if (last === guardPending) { toast('不能連續兩晚守護同一人'); return; }
...
function sendWolfChat() {
  ...
  room.night.wolfChat = Array.isArray(room.night.wolfChat)
    ? room.night.wolfChat.filter(m => m && typeof m === 'object')
    : [];
...
function renderWolfChatAndVotes(room, n, me) {
  ...
  const allVotedNow = wolves.every(w => wolfVotes[w.id] && wolfVotes[w.id] !== '_init');
  const targetsNow = wolves.map(w => wolfVotes[w.id]).filter(v => v && v !== '_init');
...
function renderWolfConfirmArea(room, n) {
  ...
  const iConfirmed = wolfConfirmedBy[myId];
  const myVote = wolfVotes[myId];
  const target = (myVote && myVote !== '_init') ? room.players.find(p => p.id === myVote) : null;
  if (iConfirmed) {
...
function renderWolfChat(room) {
  const stream = document.getElementById('wolf-chat-stream');
  if (!stream) return;
  const rawMsgs = room.night?.wolfChat || [];
  // Filter out sentinel values
  const msgs = Array.isArray(rawMsgs) ? rawMsgs.filter(m => m && typeof m === 'object') : [];
  // Stable-key render
  ...
  msgs.forEach((msg, i) => {
    ...
    div.style.cssText = 'font-size:0.82rem;line-height:1.45;padding:2px 0;max-width:65%;display:grid;grid-template-columns:auto 1fr;gap:0 4px;align-items:baseline;word-break:break-word;';
    const isMe = msg.senderId === myId;
    div.innerHTML = `<span style="font-weight:700;color:${isMe ? '#f59e0b' : '#a16207'};white-space:nowrap;">${escHtml(msg.senderName)}</span><span style="color:var(--text2);word-break:break-word;min-width:0;">${escHtml(msg.text)}</span>`;
...
function wolfVote(targetId) {
  ...
  const confirmedBy = room.night.wolfConfirmedBy || {};
  if (confirmedBy[myId]) return;
  room.night.wolfVotes = room.night.wolfVotes || {};
...
function wolfConfirm() {
  ...
  if (!wolfVotes[myId]) { toast('請先選擇目標'); return; }
...
function seerCheck(targetId) {
  ...
  if (room.night.seerCheck && room.night.seerCheck !== '_none') return;
...
function witchHeal() {
  ...
  if (!room.witchHasHeal) return;
  const wolfKill = (room.night.wolfKill && room.night.wolfKill !== '_none') ? room.night.wolfKill : null;
  if (!wolfKill) return;
  const alreadyActed = room.night.witchAction !== undefined && room.night.witchAction !== '_none';
  if (alreadyActed) return;
  witchPending = { type: 'heal', target: wolfKill };
  const content = document.getElementById('night-action-content');
  const me = room.players.find(p => p.id === myId);
  renderMyNightAction(room, room.night, me, content);
}
...
function witchSkip() {
  const room = getRoom(currentRoomCode);
  if (!room.night || room.night.step !== 'witch') return;
  if (room.night.witchAction !== undefined && room.night.witchAction !== '_none') return;
  room.night.witchAction = { type: 'skip', target: '_none' };
  witchPending = null;
  saveRoom(room);
}
...
function advanceNightStep(room) {
  ...
  if (step === 'guard') {
    ...
    const guardTarget = n.guardTarget;
    const guardDone = guardTarget !== undefined && guardTarget !== '_none';
    if (guards.length === 0 || guardDone) stepDone = true;
    ...
    if (wolves.length === 0) stepDone = true;
      ...
      if (allConfirmed) {
        ...
        const wolfVotes = n.wolfVotes || {};
        const votes = wolves.map(w => wolfVotes[w.id]).filter(v => v && v !== '_init');
        const tally = {};
        votes.forEach(v => { tally[v] = (tally[v] || 0) + 1; });
        const max = Math.max(0, ...Object.values(tally));
        const top = Object.entries(tally).filter(([, c]) => c === max).map(([t]) => t);
        n.wolfKill = top.length === 1 ? top[0] : (top.length > 0 ? top[Math.floor(Math.random() * top.length)] : '_none');
        stepDone = true;
      }
      if (!stepDone && elapsed >= limit) {
        // Timeout: use whatever votes exist
        const wolfVotes = n.wolfVotes || {};
        const allVotes = Object.entries(wolfVotes)
          .filter(([k, v]) => k !== '_init' && v && v !== '_init')
          .map(([, v]) => v);
    ...
    const seerDone = n.seerCheck !== undefined && n.seerCheck !== '_none';
    if (seers.length === 0 || seerDone) stepDone = true;
    if (!stepDone && elapsed >= limit) {
      n.seerCheck = 'skip';
      stepDone = true;
    }
    ...
    else if (n.witchAction !== undefined && n.witchAction !== '_none') stepDone = true;
    if (!stepDone && elapsed >= limit) {
      n.witchAction = { type: 'skip', target: '_none' };
      stepDone = true;
    }
  }
...
function finishNight(room) {
  ...
  const wolfKill = (n && n.wolfKill && n.wolfKill !== '_none') ? n.wolfKill : null;
  if (wolfKill) {
    const guardTarget = (n.guardTarget && n.guardTarget !== '_none' && n.guardTarget !== 'skip') ? n.guardTarget : null;
    const witchAction = (n.witchAction && n.witchAction !== '_none') ? n.witchAction : null;
    const guardSaved = guardTarget && guardTarget === wolfKill;
    const witchSaved = witchAction && witchAction.type === 'heal' && witchAction.target === wolfKill;
    ...
    if (!guardSaved && !witchSaved) {
      deaths.push({ id: wolfKill, cause: 'wolf' });
    } else if (bothProtect) {
      deaths.push({ id: wolfKill, cause: 'wolf' });
    }
  }
  ...
  // Witch poison
  const witchAction = (n && n.witchAction && n.witchAction !== '_none') ? n.witchAction : null;
  if (witchAction && witchAction.type === 'poison' && witchAction.target && witchAction.target !== '_none') {
    if (!deaths.find(d => d.id === witchAction.target)) {
      deaths.push({ id: witchAction.target, cause: 'poison' });
    }
  }

  // Update potion availability
  if (witchAction) {
    if (witchAction.type === 'heal') room.witchHasHeal = false;
    if (witchAction.type === 'poison') room.witchHasPoison = false;
  }

  // Save guard target for next night
  const guardTarget = (n && n.guardTarget && n.guardTarget !== '_none' && n.guardTarget !== 'skip') ? n.guardTarget : null;
  room.lastGuardTarget = guardTarget || '_none';
  ...
  room.night.deaths = deaths.length > 0 ? deaths : ['_empty'];
...
function startDay(room) {
  ...
  room.day = {
    ...
    speakLimit: 120,
    voteState: null,
    deaths: (room.night?.deaths || []).filter(d => d !== '_init' && d !== '_empty'),
  };
  // Add log entry — filter sentinel values before processing
  const realDeaths = room.day.deaths.filter(d => d && typeof d === 'object');
  const deathNames = realDeaths.length > 0
    ? realDeaths.map(d => {
        const p = room.players.find(x => x.id === d.id);
    ...
    : '無人死亡';
  room.log = room.log || [];
  room.log.push({ type: 'system-night', text: `🌅 第 ${room.dayNum} 天 — 昨夜${realDeaths.length > 0 ? `死亡：${deathNames}` : '是平安夜'}`, ts: Date.now() });
  saveRoom(room);
}
...
function advanceDayStep(room) {
  ...
  if (d.phase === 'review') {
    // Auto-enter vote after review duration
    if (d.reviewStart && Date.now() - d.reviewStart > (d.reviewDuration || 60) * 1000) {
      enterVotePhase(room);
    }
    return;
  }
...
function advanceSpeaker(room) {
  ...
  if (d.speakerIdx >= d.speakOrder.length) {
    // All done → 60s review before vote
    d.phase = 'review';
    d.reviewStart = Date.now();
    d.reviewDuration = 60;
    room.log = room.log || [];
    room.log.push({ type: 'system', text: '所有人發言完畢，回顧 60 秒後進入投票', ts: Date.now() });
    saveRoom(room);
  } else {
    d.speakerStart = Date.now();
    saveRoom(room);
...
function hostEndSpeechPhase() {
  ...
  if (room.day?.phase !== 'speech' && room.day?.phase !== 'review') return;
  enterVotePhase(room);
}
...
function enterVotePhase(room) {
  ...
  room.day.phase = 'vote';
  room.day.voteState = {
    votes:     { _init: 1 },    // sentinel: Firebase won't drop non-empty object
    confirmed: { _init: 1 },
    timerStart: Date.now(),
    duration: 60,
  };
...
function renderDayView(room) {
  ...
  phaseBadge.textContent = d.phase === 'speech' ? '討論時間' : d.phase === 'review' ? '回顧發言' : d.phase === 'vote' ? '投票階段' : '討論時間';
...
function updateDayView(room) {
  ...
  // During review: host sees "進入投票" button; during speech: host sees "進入投票" too
  document.getElementById('day-end-speech-host').classList.toggle('hidden', !(isHostUser && (d.phase === 'speech' || isReview)));

  // Review phase: show countdown in phase badge
  if (isReview && d.reviewStart) {
    const elapsed = Math.floor((Date.now() - d.reviewStart) / 1000);
    const remaining = Math.max(0, (d.reviewDuration || 60) - elapsed);
    phaseBadge.textContent = `回顧發言 ${remaining}s`;
  }

  // Update stream
  updateDayStream(room);

  // Disable input if I'm dead or not the current speaker during speech phase, or in review
  ...
  if (inputEl) {
    if (!me?.alive) {
      ...
      inputEl.placeholder = '回顧發言中，即將進入投票…';
      inputEl.disabled = true;
    } else if (d.phase === 'speech' && !isMyTurn) {
      inputEl.placeholder = '等待輪到你發言…';
      inputEl.disabled = true;
    } else {
...
function sendDayChat() {
  ...
  const me = room.players.find(p => p.id === myId);
  if (!me?.alive) return;
  // During speech phase, only the current speaker can send messages
  if (room.day?.phase === 'speech') {
    const curSpeakerId = room.day.speakOrder[room.day.speakerIdx];
    if (curSpeakerId !== myId) return;
  }
  room.chat = room.chat || [];
...
function updateVoteView(room) {
  ...
  const confirmedMap = v.confirmed || {};
  const votesMap = v.votes || {};
  const confirmed = confirmedMap[myId];
  const myVote = votesMap[myId];
  const allVoted = alive.every(p => confirmedMap[p.id]);

  // Tally (only show after all voted) — filter sentinel
  const tally = {};
  ...
  const votedCount = alive.filter(p => confirmedMap[p.id]).length;
...
function checkVoteCompletion(room) {
  ...
  if (alive.every(p => confirmed[p.id])) {
    resolveVote(room);
  }
}
...
function resolveVote(room) {
  ...
  const rawVotes = v.votes || {};
  const tally = {};
  // Filter out sentinel _init key
  ...
  // Build per-player vote breakdown for the result screen — filter sentinel
  const breakdown = {};
  Object.entries(rawVotes)
    .filter(([k]) => k !== '_init')
    .forEach(([voter, target]) => {
      if (!breakdown[target]) breakdown[target] = [];
      breakdown[target].push(voter);
    });

  // Save vote result for the post-vote view
  const confirmedMap2 = v.confirmed || {};
  room.voteResult = {
    breakdown,
    banishedId,
    ...
    abstained: room.players.filter(p => p.alive && !confirmedMap2[p.id]).map(p => p.id),
    deadlineToContinue: Date.now() + 5000,
  };
